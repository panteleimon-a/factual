try:
    ...
    import re
    from bert.API.tf_in_use import etl
    ...
    print("Library in Parser.py failed to get imported")
...
class ChromeSocket():
    def __init__(self, url):
        ...
        r = session.get(url,headers=headers ,allow_redirects=False ,cookies=cookies, timeout=15)
        self._encoding = BeautifulSoup(r.text, "html5lib")
...
def results(query,n_pages):
    ...
    for page in range(1, n_pages):
        ...
        driver=ChromeSocket(url).driver
        driver.get(url)
...
class Parse():
    def __init__(self, query):
        # Flatten the list generated by results
        # Specify number of pages on google search, each page contains 10 #links
        self.links=([link for page in results(query, n_pages=5) for link in page])
    ...
    def text(self):
        ...
        for url in self.links:
            # For each link, initiate new socket
            session=requests.Session()
            session.verify = False
            session.trust_env = False
            headers=get_useragent()
            ...
            text.append([etl(webtext).preprocess()])
        return text
